...
**状态：已上架。** 条目于 2026-09-23 合入市场仓库
（`data/plugins/7starsseeker__dsh-fact-check.yml`，PR #5725），市场 README 与站点由该文件生成。
本目录的文件此后是**存档副本**：后续改条目要改市场仓库里那一条并另提 PR，见文末「上架后」。
...
- 市场徽章已挂在两个 README（`https://awesome-dsh-plugin.com/badge.svg` 链到站点，写法取自市场的
  Badge 段）。**链站点根而不是单插件页**：单插件页 `/p/<owner>/<repo>/` 由站点重建时才生成，
  刚合入的那条会有延迟（本次合入次日仍是 404），链过去会 404。
